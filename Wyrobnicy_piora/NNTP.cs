...
namespace Wyrobnicy_piora
{
    public class NNTP : System.ComponentModel.Component
    {
        ...
        /// <exception cref="System.Exception">Thrown when an invalid response is received
        /// from the server</exception>
        public int DownloadHeaders(string XOVERCommand)
        { try
            {
                ...
                sw.WriteLine("newnews soc.culture.polish 201101 000000 ");
        ...
        public List<string> Retrieve_By_dates(string group)
        {
            ...
            try
            {
                string parm1 = "newnews " + Globals.newsgroup + " "  + Globals.date_from.ToString("yyMMdd") + " 000000";
                sw.WriteLine(parm1);
        ...
        public List<Email> Retrieve_10000_Headers(string group)
        {
            ...
            if (strResponse == null)
            {
                error_msg = "GROUP: NULL Response received from server, try different server";
                return null;
            }
                    ...
                    do
                    {
                        ...
                        else
                        {
                            ...
                            if  (fields[2].StartsWith("=?UTF-8?Q?") == true)
                            {
                                ...
                                new_text2 = new_text1.Replace("=C5=9B", "s");
                                new_text1 = new_text2.Substring(9, new_text2.Length - 9);
                            ...
                            if (fields[2].StartsWith("=?iso-8859-2?q") == true)
                            {
                                new_text1 = fields[2].Replace("=B3", "l"); 
                                new_text2 = new_text1.Substring(14, new_text1.Length - 15);
                                new_text1 = new_text2.Replace("?", "");
                                fields[2] = new_text1.Replace("=", "");

                            }
                            if (   (fields[2].ToUpper()).StartsWith("=?ISO-8859-2?B?") == true)
                            {
                                int i1 = fields[2].IndexOf("<");
                                int i2 = fields[2].IndexOf(">");
                                proper_email = fields[2].Substring(i1, i2 - i1 + 1);

                                new_text1 = fields[2].Substring(14, fields[2].Length - 15);
